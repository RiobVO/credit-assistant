...
def _cp(inn_value: str, registered: date) -> Counterparty:
    return Counterparty(inn=INN(inn_value), name=f"ООО {inn_value}", registration_date=registered)


class TestShellCompanyPartners:
    def test_fires_when_buyer_younger_than_6_months(self) -> None:
        # Зарегистрирован 3 мес назад
        shell = _cp("111111111", date(2026, 2, 1))
    ...
    def test_silent_when_all_older_than_6_months(self) -> None:
        old = _cp("111111111", date(2024, 1, 1))
    ...
    def test_lists_all_shell_inns_in_evidence(self) -> None:
        shell1 = _cp("111111111", date(2026, 2, 1))
        shell2 = _cp("222222222", date(2026, 3, 1))
        old = _cp("333333333", date(2020, 1, 1))
