...
**Phase:** 8 закрыта 2026-05-15 — Manual-input Step 3 (Loan) design statement DONE (`94229e8`). Три связанные section-card: «Условия кредита» (Banknote + counter N/5), DSCR pre-score (TrendingUp + static updated pill, без pulse — закрывает часть TODO[CA-DS19]), «Перед отправкой на скоринг» (ClipboardCheck + counter ok/total). Native `<select>` для loanTermMonths заменён на extracted `<CustomDropdown<number>>`; sparkbars в DSCR удалены. Shared компоненты extracted: `section-card.tsx` (SectionCard / CounterChip / StaticPill) + `custom-dropdown.tsx` — переиспользуются Step 2 / Step 3 / DSCR / Checklist. Hex sweep на semantic tokens. Schema не тронута. Phase 9 (Dossier view) unblocked. Specs: `docs/superpowers/specs/2026-05-11-phase-4-bank-mode-design.md`, ADR-0009, ADR-0011.
