...
**Next task:** 2.5.6.b frontend draft integration (TanStack Query mutation для save, debounced на переход между шагами; на mount при `?draft={id}` — попытка GET и заполнение формы) ИЛИ 2.5.7 testcontainers (real Postgres для интеграционных round-trip тестов репозиториев). Решить порядок при следующем чек-ине.

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence** (2.5.1 ✓ → 2.5.2 ✓ → 2.5.3 ✓ → 2.5.4 ✓ → 2.5.5 ✓ → 2.5.6 backend ✓ → **2.5.6.b frontend** или **2.5.7 testcontainers** ← здесь → 2.5.8 ADR + DoD) → 2.3 SoliqExcelAdapter (после первой реальной выгрузки) → 2.6 E2E на 5 фирмах. VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
