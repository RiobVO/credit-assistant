...
**Last completed task:** 2.5.6.b frontend draft integration — `useFormDraft` hook (`web/src/app/(accountant)/manual-input/_hooks/use-form-draft.ts`) оборачивает react-hook-form, делает POST на первый save, PUT на последующих, fallback POST при PUT-404 (TTL истёк / draft удалён). Save fire-and-forget на переходе шагов 1→2 / 2→3 — переход не блокируется сетью. Mount: `?draft={id}` в URL → GET → `form.reset(payload)`; 404 / битый id → query-param чистится через `window.history.replaceState`. Topbar получил optional `draft: DraftIndicator` (saving / saved / error) с бейджем «Черновик сохранён HH:MM» слева от Search. Page обёрнута в `Suspense` — Next App Router требует boundary вокруг `useSearchParams` при prerender. `web/src/lib/api.ts` расширен: `createDraft / updateDraft / getDraft` + DTO; общий `jsonFetch` хелпер. Verify: `eslint` + `tsc --noEmit` + `next build` зелёные; smoke против compose-postgres подтвердил полный цикл `POST → GET → PUT → GET-after-put → 404` через bare API. Клиентская логика (form.reset/replaceState/индикатор) проверена только сборкой и HTTP 200 на трёх URL — ручной браузерный smoke остался за пользователем. Коммит `b9f1468` запушен.
**Next task:** 2.5.7 testcontainers — интеграционные тесты репозиториев и end-to-end manual-input + draft flow против real Postgres из testcontainers (см. договорённость в CLAUDE.md). Сейчас mapper-тесты в 2.5.4 без БД, integration-тесты dossier/draft через in-memory dummies. Нужны: фикстура `pg_engine` через `testcontainers.PostgresContainer`, applied alembic head на старте, скоуп — `tests/integration/persistence/`. После 2.5.7 → 2.5.8 ADR 0005 (drafts auth-by-uuid) + DoD-чеклист Phase 2.

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence** (2.5.1 ✓ → 2.5.2 ✓ → 2.5.3 ✓ → 2.5.4 ✓ → 2.5.5 ✓ → 2.5.6 backend ✓ → 2.5.6.b frontend draft ✓ → **2.5.7 testcontainers** ← здесь → 2.5.8 ADR + DoD) → 2.3 SoliqExcelAdapter (после первой реальной выгрузки) → 2.6 E2E на 5 фирмах. VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
