...
**T3.6 (On-prem tarball deploy) complete 2026-05-18** (commits `9a4cef1` + `eaecec5` + `5684f22` + `a3c2d1a` + this). 5 атомарных коммитов. **Deal-breaker** zero-internet install для узб mid-tier банков closed. ADR-0021. **T3.6.1** `web/Dockerfile` multi-stage (deps/builder/runner) + `next.config.ts` `output: "standalone"` — self-contained server.js без npm/dev-deps, image ~200-250 MB. USER nextjs (UID 1001 non-root), NEXT_TELEMETRY_DISABLED. **T3.6.2** `deploy/install.sh` (bash installer: preflight → .env validation `JWT_SECRET ≥32` / `PII_ENC_KEYS` обязателен в prod / `BRAND_ID` резолвится / `LDAP_*` если ldap → `docker load` images/ → systemd install + enable + start → wait /health 90s + onboarding instructions). `deploy/.env.example` (все обязательные env с placeholder'ами и validation rules). `deploy/Caddyfile.template` (reverse proxy `/api/*` + `/health` → api:8000, прочее → web:3000, HSTS / X-Frame / Referrer-Policy baseline; LE для internet-facing, mounted cert для bank-internal CA). `deploy/systemd/credit-assistant.service` (Type=oneshot RemainAfterExit, ExecStart docker compose up). `deploy/systemd/credit-restore-drill.{service,timer}` (Sun 04:00 Asia/Tashkent weekly). `docker-compose.prod.yml` override: postgres/redis без host-portов (только internal compose-network), api/web `image:` не build, новый caddy service ports 80/443. **T3.6.3** `scripts/build_release_tarball.sh` — bundles credit-assistant-vX.Y.Z.tar.gz: 5 docker save | gzip-9 images (api/web/caddy/postgres/redis) + source + scripts/ + config/ + deploy/ + compose files + alembic + docs/operations + docs/adr + SHA256. Approx size 600 MB - 1.5 GB. Не включает tests/.git/dist/node_modules/.env/*.dump/photos. **T3.6.4** ADR-0021 (context: банки без registry/K8s + zero-internet + compliance reproducibility; alternatives rejected: registry-pull / Ansible / K8s / apt-repo). `deploy/README.md` "0 → running" walkthrough <30 мин: preconditions / extract + secrets / install.sh / verify (curl /health, seed admin) / upgrade procedure / troubleshooting (Caddy port, migrations, docker load) / uninstall soft+hard. **Tier 3 — 3 deal-breaker'а closed (T3.2/T3.4/T3.6). Остались nice-to-have: T3.1 observability (GlitchTip on-prem) / T3.3 Prometheus / T3.5 audit-export CSV.**

**T3.4 (Postgres backup + restore drill) complete 2026-05-18** (commits `4f333c5` + `7ccdbba` + `e5fdd98` + this). 4 атомарных коммита. **Deal-breaker** для bank-IT onboarding closed. `scripts/backup_postgres.sh` — bash, `pg_dump --format=custom --compress=9 --no-owner --no-acl` + retention `find -mtime +N` (default 7d dev / 30d prod через env). Encryption opt-in через `BACKUP_AGE_RECIPIENT` (age / filippo.io — modern asymm, single binary, no keyring) — default off в dev, on в prod через Vault recipient. Exit-codes контракт 10/11/12 (pg_dump / age / retention). `scripts/restore_drill.sh` — pg_restore latest в temp DB → COUNT(*) per table source vs restored через `query_to_xml(format(...))` (точнее n_live_tup из pg_stat_user_tables, который ANALYZE-зависим) → PASS если max diff < `DRILL_ROW_DIFF_PCT` (default 5%). Cleanup temp DB через `trap EXIT` always-fire. Exit codes 0/20/21/22/23/24. `docker-compose.backup.yml` override — sidecar `db-backup` на `postgres:16-alpine` (major-version match), `apk add dcron age` на entrypoint, crontab `0 2 * * *`. `docs/operations/db-backup.md` — полный playbook: 3 prod destinations (A local / B NFS / C S3-compat MinIO) для onboarding-choice банка, encryption setup через age-keygen + Vault, disaster scenarios (corrupted pgdata / loss PII_ENC_KEYS / corrupted backup), operational checklist (daily/weekly/monthly/quarterly), restore drill systemd timer для weekly schedule. Tests: 3 integration backup (testcontainers) + 3 integration restore drill. **※ heads-up**: текущий CI workflow без Docker job → integration tests фактически skipped в CI (lint+mypy pass). Smoke на dev-Docker до production deploy обязателен. Setup Docker-action — post-demo hardening (T3.x).
...
4. **T3.1** Observability — GlitchTip on-prem (Sentry SaaS нельзя — PROJECT_BRIEF Sec 8) либо plain logs + SIEM. Backend + frontend `error.tsx` hook. ADR-0022. Зависит от T3.2 (correlation_id для tags).
