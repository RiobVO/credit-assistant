...
- TODO[CA-067]: удалить `back-target.ts` + вызовы `rememberBackTarget()` в `(bank)/search`/`(bank)/history` — orphaned после CA-066 (нижний ActionBar удалён, `consumeBackTarget()` некому звать). Оставлено если вернём back-кнопку в шапку.
