...
**Last completed task:** 2.5.4 repositories + mappers — 4 Protocol-порта в `application/ports/` (`BorrowerRepositoryPort`, `BorrowerSnapshotRepositoryPort`, `DossierRepositoryPort`, `DraftRepositoryPort`), DTO `application/dto/dossier_record.py` (domain не имеет сущности Dossier — record это `score/recommendation/severity_breakdown/red_flags/rules_version/rules_evaluated`). Mappers в `infrastructure/persistence/mappers/`: borrower (1:1 + Money split), snapshot (детерминированный JSONB: Decimal→str, date→ISO, enum→.value, INN→.value), dossier (RedFlag ↔ JSONB). SQLAlchemy-impl: Borrower через postgres `INSERT ... ON CONFLICT (inn) DO UPDATE` (атомарный UPSERT, returns UUID); Snapshot insert + чтение с join borrower; Dossier insert/get; Draft CRUD с TTL `expires_at = now() + draft_ttl_days`, expired записи фильтруются на чтении и удаляются `purge_expired()`. 8 round-trip mapper-тестов (без БД) — full snapshot, empty snapshot, JSON-сериализуемость payload, Decimal precision, RedFlag round-trip, borrower full+minimal, kwargs-без-id. Интеграционные тесты репозиториев — отложены на 2.5.7 (testcontainers). ruff + mypy --strict + 272 теста — зелёные. Коммит `fd45abf` запушен.
**Next task:** 2.5.5 wire endpoint + CA-005 — `POST /api/manual-input` после use case вызывает: `borrower_repo.upsert(...)` → `snapshot_repo.save(...)` → `dossier_repo.save(...)`, возвращает `dossier_id` в `DossierResponse`. DI через FastAPI `Depends` + `AsyncSession` per request. Расширить `ManualInputRequest` полями `loan_term_months: int`, `loan_rate_pct: Decimal`, `loan_purpose: str`, `loan_category: str` (UI на Шаге 3 их собирает, но в payload пока не пускает). На Windows uvicorn-startup нужен `WindowsSelectorEventLoopPolicy` для рантайма asyncpg.

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence** (2.5.1 ✓ → 2.5.2 ✓ → 2.5.3 ✓ → 2.5.4 ✓ → **2.5.5 wire endpoint + CA-005** ← здесь → 2.5.6 drafts → 2.5.7 testcontainers → 2.5.8 ADR + DoD) → 2.3 SoliqExcelAdapter (после первой реальной выгрузки) → 2.6 E2E на 5 фирмах. VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
