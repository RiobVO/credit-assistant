...
5. **ADR-0024 backlog** (требуют расширения domain entities / новых каталогов — см. ADR-0024 «Research debt remaining»):
   - **Session 3 KPI `fx_exposure_ratio`**: требует currency breakdown на `BalanceSnapshot.liabilities_total` (новая entity `Liability` или расширение `Counterparty` / `LoanRequest` с currency). Парсеры FORM_1 + snapshot_mapper round-trip + UI/PDF mapping.
