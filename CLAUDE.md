...
**Phase:** 2 — Data Adapters. Подфаза 2.5 (persistence) **смержена в `main`** через PR #1 (merge commit `5400f9e`). 2.3 + 2.6 — в pending-external-data.
**Last completed task:** Phase 2.5 closure — PR #1 «Phase 2.5 — Persistence layer» смержен в `main`. На main приехали 27 коммитов / +8611/−138 за период 2.0 → 2.5.8. Ветка `feat/phase-2-data-adapters` локально удалена (вся история в merge commit). Remote-ветка живёт пока — удалять ли её, решит пользователь (стандартный safety: смержена, безопасно удалить через `git push origin --delete feat/phase-2-data-adapters`).
**Next task:** ждём внешний trigger. Когда папа пришлёт первую реальную выгрузку Soliq Excel — стартуем 2.3 SoliqExcelAdapter в новой ветке `feat/soliq-excel-adapter` от свежего main. Альтернативно, если придёт сводная справка НДС, можно стартовать VAT-адаптер для активации `VAT_ESF_MISMATCH`. Phase 4 (Bank Mode + SSO) — отдельный roadmap, за рамки текущего pending-окна.

**Phase 2 декомпозиция (статус):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence ✓ → main** (2.5.1 → 2.5.2 → 2.5.3 → 2.5.4 → 2.5.5 → 2.5.6 → 2.5.6.b → 2.5.7 → 2.5.8) → 2.3 SoliqExcelAdapter ⏸ (pending: первая реальная выгрузка от папы) → 2.6 E2E на 5 фирмах ⏸ (pending: данные 5 фирм). VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
