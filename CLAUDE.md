...
- TODO[CA-033] (P1): pre-score gauge на Шаге 3 (`web/.../step-3`) — когда `dscr === null`, pill должен быть серый «Недостаточно данных», а не красный «Высокий риск»; текст «норма ≥ 1.25× · покрытие 0%» скрывать. Acceptance: 3 ветки (null/grey, <1.25/red, ≥1.25/green) + unit test `pre-score.test.tsx`.
...
- TODO[CA-035] (P1): чек-лист «Перед отправкой на скоринг» (Шаг 3): (1) пункт «Активы и обязательства соответствуют форме №1» ставит галку только если `source_trail` содержит ключ `form1.*` — иначе amber «Введено вручную (FORM_1 не загружен)»; (2) счётчик «X/24» учитывает autofilled cells, не только manual. Acceptance: unit на 4 состояния (нет данных / manual / autofilled FORM_2 / autofilled FORM_2+FORM_1).
- TODO[CA-036] (P2): копирайт «Выручка по месяцам» — заменить empty state на «Помесячная динамика недоступна — загрузите ESF CSV (CA-032)». Сейчас «Нет помесячных данных» при заполненном KPI «Выручка LTM −14.4%» — противоречие.
