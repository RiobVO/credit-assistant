...
**Last completed task:** 2.5.5 wire endpoint + CA-005 + LoanRequest VO — `POST /api/manual-input` стал async, после use case вызывает `storage.borrower.upsert()` → `storage.snapshot.save()` → `storage.dossier.save()` под одной транзакцией (`get_session` обёрнут в `session.begin()` — auto-commit/rollback). `DossierResponse` получил `dossier_id: UUID`. `DossierStorage` — DI-контейнер 4 репозиториев, в тестах оверрайдится на in-memory dummies (без БД). `engine` получил `json_serializer` с обработкой Decimal→str (rule evidence несёт Decimal, stdlib `json.dumps` его не умеет). `interfaces/api/main.py` ставит `WindowsSelectorEventLoopPolicy` при импорте (до создания asyncpg-соединения). Refactor: `BorrowerSnapshot.loan_request_amount` → `loan_request: LoanRequest | None`; новый VO `LoanRequest(amount, term_months, rate_pct, purpose, category)` в `domain/value_objects/loan_request.py`; правило `LOAN_TO_REVENUE_RATIO`, mappers, fixtures, pydantic-схема (`LoanRequestInput`), frontend `_form-mapper.ts` обновлены end-to-end. CA-005 закрыт. Smoke: uvicorn → POST → 200 + `dossier_id`, psql показывает по 1 строке в borrowers/snapshots/dossiers. ruff + mypy --strict + 276 тестов — зелёные. Коммиты `ce49b93` (refactor) + `1301525` (wire) запушены.
**Next task:** 2.5.6 drafts — `POST /api/manual-input/draft` (создание, возвращает draft_id), `PUT /api/manual-input/draft/{id}` (продление + апдейт payload), `GET /api/manual-input/draft/{id}` (восстановить — null если истёк). UI: на переход между шагами debounced save в draft, на mount при наличии draft_id в URL — попытка восстановить. Подключить `DraftRepositoryPort` через `DossierStorage`. Schema: payload — частичный `ManualInputRequest` (все поля Optional, без валидации обязательных полей).

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence** (2.5.1 ✓ → 2.5.2 ✓ → 2.5.3 ✓ → 2.5.4 ✓ → 2.5.5 ✓ → **2.5.6 drafts** ← здесь → 2.5.7 testcontainers → 2.5.8 ADR + DoD) → 2.3 SoliqExcelAdapter (после первой реальной выгрузки) → 2.6 E2E на 5 фирмах. VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
