...
Pre-demo MVP ready. **ADR-0024 Session 3 closed (2026-05-20, PR #11)** — 5
Tier 2 rule narrows + 5 entity полей + 1 Alembic migration + UI Step 1/3
обновлены. Готов к pilot trip.
...
infrastructure. Подробности `docs/adr/0024-foundational-source-verification.md`
+ research outputs `docs/research/2026-05-19-3way-reconcile/`.
...
5. **ADR-0024 backlog** (требуют расширения domain entities / новых каталогов — см. ADR-0024 «Research debt remaining»):
   - **`fx_exposure_ratio` KPI**: требует currency breakdown на `BalanceSnapshot.liabilities_total` (новая entity `Liability` или расширение `Counterparty` / `LoanRequest` с currency). Парсеры FORM_1 + snapshot_mapper round-trip + UI/PDF mapping.
   ...
   - **OKVED_CHANGED_12M brand-new dossier flow**: Session 3 закрыла owner-gate, но UI toggle reachable только во flow «Пересобрать с дополнениями» (требует parser-given `okved_main_changed_at`). Brand-new dossier flow покроет Госкомстат ОКЭД-API integration (Tier 3 rename).
