...
**Phase:** 9 закрыта 2026-05-14 — Dossier view design statement DONE (`bcde558`). SectionCard перенесён в global `web/src/components/section-card.tsx` с optional `icon` prop: wizard surfaces (Step 1-3) передают icon-tile, dossier surfaces — не передают (banking-минимализм для read-only). 4 секции dossier обёрнуты в SectionCard: BorrowerCard (verified pill убран до CA-003, TODO[CA-DS28]), ScoreGauge, Revenue24mChart (period-selector как aux), RiskSignals (CounterChip {flags.length / rules_evaluated} как aux). KpiCard — sparkline блок удалён физически (backend никогда не заполнял, 3 годовые точки FORM_2 = ломаная не волна; TODO[CA-DS25] до monthly EBIT projection). KPI row плотнее ~120→100 px. SubHeader status-eyebrow «ЗАЯВКА BR-… · ДАННЫЕ НА dd.mm.yyyy» в ink-4 (не brand — read-only metadata не тянет глаз); buttons 38→40 px + rounded-md→rounded-[9px]. RiskSignals severity-pills hex → semantic state-tokens; accordion `border-l-2 brand-primary` + `surface-2` bg (gradient убран — рич для evidence-dl); source-line в footer expanded-block. i18n: + eyebrow_application/eyebrow_as_of, + section_title/section_sub, + counter_eyebrow; risk.title «Сигналы риска» → «Красные флаги»; orphan keys (`borrower_card.title/inn_prefix/verified`, `score.label`) удалены. Phase 10 (PDF document) unblocked. Specs: `docs/superpowers/specs/2026-05-11-phase-4-bank-mode-design.md`, ADR-0009, ADR-0011.
...
- TODO[CA-DS19]: motion cleanup pass по /search и /history. В Phase 2/3 остались pulse-ring-ok dots на trust-pill «Все системы работают» и LiveStrip — user в Phase 6 явно попросил «без вау-эффекта который горит в реальном времени, это банк». Sweep + revert pulse-* анимаций, оставить только user-card sidebar online-dot (semantic). Phase 8 закрыл DSCR-header pulse-dot (был на «обновлено · сегодня»), остаток — /search trust-pill + LiveStrip.
...
- TODO[CA-DS24]: real `/api/system/cbu/usd-rate` или config-driven course вместо hardcoded `USD_RATE_UZS = 12575` в `step-3-loan.tsx`. Сейчас USD-equivalent в help-line «≈ {usd} USD по курсу ЦБ РУз (1 USD = 12 575 UZS)» — фиксированное значение, устареет через неделю. Низкий приоритет — это help text, не critical-данные.
- TODO[CA-DS25]: real backend sparkline для KPI (EBIT / ROE / Debt-to-EBIT). Сейчас `KpiValueOutput.sparkline: list[str]` всегда `[]` — frontend KpiCard sparkline-блок удалён физически в Phase 9. Backend `kpi_calculator` имеет только 3 годовые точки FORM_2 (2023/2024/2025) = ломаная, не волна. Нужна monthly-проекция EBIT: либо derive из `monthly_revenue_24m` × последняя annual margin (грубо), либо реальный monthly EBIT из ESF/онлайн-ККМ. После — вернуть `<Sparkline data={kpi.sparkline}/>` блок в `kpi-card.tsx` (5 минут восстановление, контракт `KpiValueDto.sparkline` остался).
- TODO[CA-DS28]: real ГНК lookup для CA-003 (hybrid scope, ~3-4 рабочих дня, **отдельный feature ticket после Phase 10**, **legal review обязателен**). v1: public lookup `soliq.uz/services/search/` через scraping в `infrastructure/adapters/gnk_public_lookup/` (httpx + BeautifulSoup, кеш 24h в Postgres, rate-limit 1 req/sec, fallback на формат-валидацию). v2: manual upload справки ГНК (`POST /api/dossier/{id}/documents` + UI dropzone в DossierView). Pill 3-state в BorrowerCard: «Найден в реестре ГНК» (зелёный, name+ОПФ match) / «Не совпадают данные» (warn, ИНН найден но имя/ОПФ stale) / «Не найден в реестре» (red). Pre-condition: уз-юрист 30 мин на «scraping публичного лукапа = OK?», проверка `robots.txt` soliq.uz. До закрытия — pill в BorrowerCard не рендерится (Phase 9 убран физически из i18n + кода).
...
- **Phase 7 CustomDropdown (SoliqUpload):** native `<select>` для year/month заменён на `<button>` + `<ul role="listbox">` поверх Base UI-style popover. Generic `<CustomDropdown<T>>` принимает `{label, value, options, onChange}`. Outside-click close через `useEffect` listener на `mousedown`. Keyboard nav — TODO[CA-DS22] (сейчас только mouse). **Phase 8: extracted в `components/custom-dropdown.tsx` (shared с Step 3 loanTermMonths).** `label` стал optional — когда компонент рендерится внутри `<Field>`, label ставит обёртка → передавать не нужно. Если label is undefined, popover top shift'ится с `top-[68px]` на `top-[48px]` (нет label = меньше offset).
- **Phase 8 SectionCard / CounterChip / StaticPill (shared shell):** Extracted в `components/section-card.tsx` после третьего consumer'а (Step 2 ×3 + Step 3 + DSCR + Checklist). Visual-only компоненты без i18n bindings — `title: ReactNode`, `sub: ReactNode`, `aux: ReactNode` (generic slot для counter / static pill / прочих). CounterChip: `{filled, total, eyebrow}` с auto-color-switch на `var(--state-ok-fg)` когда `filled >= total` (done state). StaticPill: dot + label на `state-ok-{bg,border,fg}` для «обновлено · {date}» без motion (Phase 8 анти-pulse для банка). **Не дублировать pattern локально** — все 5+ consumer'ов через shared.
...
| 2026-05-14 | Phase 9 | **Dossier view design statement DONE (`bcde558`).** Preview HTML `web/design-reference/2026-05-15-dossier-phase9-preview.html` с 3 сценариями вдоль скролла (happy / partial / risk-heavy). После первого критического review (reality-check на banking UX) применили 4 правки до implementation: (1) убрали misleading «Проверено в ГНК» pill — был mock по 9-знач. валидации формата, не реальный ГНК-lookup → дезинформация на decision-screen аналитика; (2) status-eyebrow brand-primary → ink-4 — read-only metadata не должна тянуть глаз (CTA-цвета только для actionable/clickable, ошибочно применил Phase 6 stepper pattern); (3) section-card без icon-tile на dossier — banking-минимализм Bloomberg/Tinkoff Бизнес; wizard'ы оставили с icon (там user взаимодействует, на dossier — сканирует); (4) accordion expanded-block: gradient `brand-primary-soft → transparent` → чистый `surface-2` + `border-l-2 brand-primary` — Phase 4 FAQ pattern работал на эмоциональном answer-тексте, на evidence-dl рич. **Файлы (17 — 1 new + 1 rename + 15 modified):** `web/src/components/section-card.tsx` (new, moved из `features/manual-input/components/`; `icon` prop optional → wizard передаёт icon-tile, dossier — нет, header grid схлопывается `40px_1fr_auto` → `1fr_auto`); 4 Phase 8 callsites обновили импорт на `@/components/section-card` (`checklist`, `dscr-summary`, `step-2-financials`, `step-3-loan`); dossier 10 файлов (sub-header rewrite с status-eyebrow + buttons 40/rounded-[9px]; borrower-card в SectionCard без pill; score-gauge в SectionCard; kpi-card без sparkline блока + recharts imports; kpi-row sparkline-prop removal × 3 + h-[36px] placeholder × 2 removal; readiness-badge h-[36px] removal; revenue-24m-chart в SectionCard с period-selector как aux + EmptyChart; risk-signals в SectionCard с CounterChip + semantic severity-pills + accordion clean + source-line в footer; dossier-skeleton KPI heights 120→100; dossier-view passes `application`/`asOf` props в SubHeader). **i18n:** + `dossier.sub_header.eyebrow_application` / `eyebrow_as_of` (ru+uz); `action_rebuild` «Пересобрать» → «Пересобрать с дополнениями»; + `borrower_card.section_title` / `section_sub`; убраны orphan `borrower_card.title/inn_prefix/verified` (verified — критично, чтобы будущие переводы не вернули misleading); + `score.section_title` / `section_sub`; reco-labels короче («Рекомендация: одобрить» → «К одобрению»); убран orphan `score.label`; + `risk.counter_eyebrow`; risk.title «Сигналы риска» → «Красные флаги»; subtitle переписан «Сработало X правил из 19 в реестре». **Engineering calls по KpiCard sparkline:** backend `KpiValueOutput.sparkline: list[str]` всегда `[]` для EBIT/ROE/Debt (3 годовые точки FORM_2 = ломаная, не волна). YAGNI: «UI которая никогда не отрисуется = удалить физически». Reversible: контракт `KpiValueDto.sparkline` в API остался, frontend просто не консумит; вернуть 5 минут когда backend подаст monthly EBIT projection (TODO[CA-DS25]). Прецедент: Phase 8 убрал sparkbars в DSCR по той же причине. **Engineering call по ГНК pill (CA-003 roadmap):** не смешиваем design-sweep с feature-work. Pill убран физически + добавлен TODO[CA-DS28] на hybrid решение (~3-4 рабочих дня отдельный ticket, **legal review обязателен**): public lookup `soliq.uz/services/search/` через scraping (httpx + BS4, кеш 24h, rate-limit 1 req/sec) + manual upload справки ГНК с OCR. Pill 3-state «Найден / Не совпадают / Не найден» — после закрытия CA-DS28. Pre-condition: уз-юрист 30 мин на «scraping публичного лукапа = OK?» + проверка robots.txt soliq.uz. **Verify:** tsc clean + eslint 0 warnings + vitest 101/101 (14 files, без новых) + next build 24 routes (без новых). Backend не тронут (ruff/mypy/pytest не нужно). CI run 25888968381 → success 1m1s. **Lessons:** (1) Read-only data screens **не** = wizard surfaces. Phase 6/7/8 паттерны (icon-tile, brand-eyebrow, gradient accordion) придумывались для **interactive flows** — переносить 1-в-1 на dossier было неправильно. Reality-check user'а сэкономил коммит-revert после catch'а. (2) Mock-pills на decision-screens (где аналитик принимает решение) — категорически опаснее чем на форме-ввода: на форме user видит mock-фидбек и понимает что система проверяет; на дотье он **доверяет** «зелёной галке проверено». Если данные mock — pill должен либо быть **точным до семантики** (например «Формат ИНН ОК»), либо вообще отсутствовать. (3) `as_of: date` ≠ `created_at: datetime`. Когда хочешь показать «когда создано» — проверь DTO. На Phase 9 я хотел показать «СОЗДАНО dd.mm.yyyy», но backend отдаёт только `as_of` (snapshot date) → переименовал eyebrow в «ДАННЫЕ НА dd.mm.yyyy» — honest по семантике, нет backend change. Сохранено в memory `feedback_dto_field_semantics.md`. (4) Shared shell extraction — при 5+ consumer'ах переноси в global `components/`, не оставляй в `features/<feature>/components/`. Phase 8 был «3 consumer'а — extract в feature», Phase 9 показал что Phase 6/7/8 + Phase 9 = 10+ consumer'ов → global нужен. **Open TODOs:** TODO[CA-DS25] (real backend sparkline), TODO[CA-DS28] (hybrid CA-003 ГНК lookup + manual upload). Существующие CA-DS19/21/22/23/24 — без изменений. Phase 10 (PDF document) — финальная фаза Design Sweep, unblocked. |

| 2026-05-15 | Phase 8 | **Manual-input Step 3 (Loan) design statement DONE (`94229e8`).** Один preview-cycle без переделок (`web/design-reference/2026-05-15-step3-phase8-preview.html`). Финальный scope: три связанные section-card вместо одной плоской — «Условия кредита» / DSCR pre-score / «Перед отправкой на скоринг». Schema invariant (5 required в zod) не тронута, чисто UI rewrite. **Файлы (9 — 7 modified + 2 new):** `step-3-loan.tsx` (rewrite с `<SectionCard>` shell + live `<CounterChip>` N/5 + `<CategoryBlock>` nested на surface-2; native `<select>` для term → `<CustomDropdown<number>>`), `dscr-summary.tsx` (rewrite — section-card с TrendingUp + `<StaticPill>` «Обновлено · {date}» без pulse-dot; Sparkbars удалены физически), `checklist.tsx` (rewrite — section-card с ClipboardCheck + counter ok/total где okCount = (innValid ? 1 : 0) + (level standard/comprehensive ? 1 : 0); hex tri-state palette → semantic state-tokens), `components/section-card.tsx` (new — `SectionCard` + `CounterChip` + `StaticPill` visual-only без i18n bindings), `components/custom-dropdown.tsx` (new — extracted из soliq-upload с optional label-prop), `soliq-upload.tsx` (cleanup: убраны local CustomDropdown + ChevronDown + useEffect imports → import из shared), `step-2-financials.tsx` (заменены inline `SectionCard`/`CounterChip` на shared imports; sigatures `titleKey/subKey/counter` → `title/sub/aux`; `eyebrow` prop добавлен в `<RevenueCounter>`/`<ProfitCounter>`), `i18n/ru.json` + `i18n/uz.json` (новые keys: `s3_counter_filled`, `s3_category_eyebrow`, `dscr_pre_score_eyebrow`, `dscr_static_updated`, `checklist_section_title`, `checklist_section_sub`, `checklist_counter_eyebrow`; удалены устаревшие `dscr_pre_score`, `dscr_summary_title`, `dscr_updated`, `dscr_annuity_today`, `s3_category_label`, `s3_calc_updated`, `checklist_heading`). **Shared extraction rationale:** 5+ consumer'ов SectionCard/CounterChip (Step 2 ×3, Step 3, DSCR, Checklist) — extract'или после третьего consumer'а, как best-practice. CustomDropdown — 2 consumer'а (soliq-upload year/month, Step 3 term), но шаблон Phase 7 был уже local → extracted как parity с Phase 6/7 sweep. **Hex sweep:** ~20 замен в 4 файлах — `#FAFBFC`/`#F4F6F9`/`#EFF1F5` → `var(--surface-2)`, `#A8C0EE` → `color-mix(in srgb, var(--brand-primary) 45%, transparent)`, `#FCE7E5`/`#F2BCBA` → `var(--state-bad-{bg,border})`, `#BFE2D2` → `var(--state-ok-border)`, `#FFF6E5`/`#F1D9A6` → `var(--state-warn-{bg,border})`, hardcoded `rgba(30,85,201,0.15)` ring → `var(--brand-primary-ring)`, `#EAEDF2` track → `var(--chart-track-light)`. ESLint hex-guard не ловит Tailwind utility-class — гигиена ручная. **Counter logic:** Step 3 — 5 предикатов `filled` (loanAmount digits>0, termMonths>0, ratePct non-empty, purpose ≥20 симв matches zod min, category всегда truthy default). Checklist — okCount/totalRows где warn/pending не считаются ok; missing_capabilities добавляют warn rows в total. **Phase 7 `<CustomDropdown<T>>` extraction story:** локальный generic в soliq-upload оставался 1-consumer (year/month). Phase 8 нужен был premium dropdown для loanTermMonths → точно та же ситуация: extract с optional `label` prop (Step 3 имеет `<Field>` обёртку → label передаётся снаружи, popover `top-[48px]` без label vs `top-[68px]` с label). **Verify:** tsc clean, eslint 0 warnings, vitest 101/101 (без новых, 14 files), next build 24 routes (без новых). Backend не тронут. CI run 25887254792 → success 1m4s. **Lessons:** (1) Когда чистишь imports — внимательно проверь, что `ReactNode` ещё не нужен файлу (step-2 имел 4 inline-определения `ReactNode` после extract'а SectionCard, забыл `import type`). TSC поймал моментально. (2) В preview HTML я нарисовал Checklist counter «3/4 done-state зелёный» — но в коде это неверная logic (warn не считаются ok; реальный counter будет «2/4» в этой ситуации). Counter в коде = source of truth, preview = visual hint. (3) Single-cycle preview-approval — впервые без правок: подсказывает что Phase 6/7 паттерны устоялись, можно применять их к похожим surface'ам без user-feedback итераций. **Open TODOs:** TODO[CA-DS24] (real /api/system/cbu/usd-rate вместо hardcoded `USD_RATE_UZS = 12575` в step-3-loan). Существующие CA-DS19/21/22/23 — без изменений (CA-DS19 частично закрыт: DSCR pulse-dot убран; остаток /search trust-pill + LiveStrip). |
