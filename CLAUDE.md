...
Закрыто 15 из 15 Design Sweep tail items 🎉. Frontend stack: 21 test files, 184 vitest tests. Backend (после T0.4): 793 pytest, 2 skipped (WeasyPrint GTK runtime), 4 xfailed (T2.3 profit_tax). Открытые TODO с pre-conditions: **CA-003** (real ГНК lookup ждёт CA-DS28 legal review), **CA-DS18b** (Postgres sequence case_id ждёт Phase 4 application entity), **CA-DS18c** (formatCaseId year edge ждёт `draft.created_at` через useFormDraft), **CA-DS25** (KPI sparkline ждёт CA-029b cashflow parser), **CA-DS28** (ГНК ждёт legal review).
...
**Tier 0 закрыт (кроме T0.1 fixtures-набора 22/20).** Next active — Tier 1 (case_id sequence, refresh-token rotation, PII encryption, multi-tenant isolation, LDAP/OAuth).
