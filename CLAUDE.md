...
**Активная ветка:** `main` (HEAD `4b89873`). Phase 4 закрыта линейно (см. Session Log); post-4 серия CA-034/CA-035/CA-041/CA-036+043+044+045+046/**CA-037** — fix-батчи по UI/PDF/data-integrity после реального smoke на досье папы. Готовы стартовать Phase 5 (E2E на 5 фирмах папы / 2.6, либо переход к pilot-демо).

**Verify status:** ruff + mypy --strict (225 src files) + 693 passed (5 PDF-тестов skip на Windows-host — нужен GTK runtime в Docker) + tsc + eslint + next build (14 routes) — зелёные. Docker smoke: `APP_MODE=bank` login→search→list+audit подтверждён ранее; CA-044 runtime smoke (POST /api/manual-input без taxes_paid[2024] → HTTP 200) на образе `7edf363`; CA-037 end-to-end smoke (parse FORM_1+FORM_2 → POST /api/manual-input → GET dossier KPI) на real fixture папы (ИНН 201308534): EBIT 256,9 млн (YoY −40,0%), ROE 10,7%, Debt/EBIT 2,41×. CI зелёный.
...
- TODO[CA-048] (P2): KPI threshold coloring в досье/PDF — `Debt/EBIT <2×` green / `2–4×` yellow / `>4×` red; `ROE >15%` green / `5–15%` yellow / `<5%` red. Сейчас KpiCard рисует серый/нейтральный фон для всех значений. Нужны палитра + категоризация на frontend и в pdf_renderer.
- TODO[CA-049] (P2): red-flag rule `NEGATIVE_EQUITY` — отрицательный или нулевой собственный капитал. Source: Базель III IRB / внутренние методики банков. Evidence: `equity_avg` числовое значение. Severity: critical. KPI calculator возвращает `roe = None` при `equity_avg ≤ 0`, но user'у нужен явный сигнал в Разделе F (а не пустая карточка ROE с подсказкой).
