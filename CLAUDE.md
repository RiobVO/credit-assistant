...
**Last completed task:** 2.5.7 testcontainers integration tests — `tests/integration/conftest.py`: session-scope `PostgresContainer("postgres:16-alpine")`, `alembic upgrade head` через `DATABASE_URL` env-var + `get_settings.cache_clear`, function-scope `pg_engine` (pytest-asyncio закрывает loop после каждого теста — engine session-scope ловил «Event loop is closed»), `pg_session` на outer-tx + savepoint (`join_transaction_mode="create_savepoint"`). Авто-skip если Docker недоступен. 17 новых тестов: borrower (UPSERT insert/update path, get_by_inn/id), snapshot+dossier (JSONB round-trip Decimal/date, FK ON DELETE RESTRICT), draft (create/get/update/TTL backdating/purge), api/manual_input_e2e (POST создаёт borrower+snapshot+dossier; draft full cycle включая 404). **Bug fix в попутку:** `_json_default` в `database.py` не умел date/datetime — любое правило с date в evidence роняло save. Добавлено `isinstance(obj, datetime | date) → isoformat()`. Marker `integration` в pyproject; README с `pytest -m "not integration"` (1.35s) и `pytest -m integration` (~5s warm). Verify: ruff + mypy --strict (160 files) + 300 тестов зелёные. Коммит `a60c87f` запушен.
**Next task:** 2.5.8 — ADR 0005 (drafts auth-by-uuid: контракт TTL=30d, без owner до Phase 4 Bank Mode, ротация через purge job — пока ручной cron) + DoD-чеклист закрытия Phase 2 persistence (testcontainers ✓, миграция применима/откатима ✓, json_serializer Decimal+date ✓, e2e через real Postgres ✓, что осталось). После 2.5.8 → 2.3 SoliqExcelAdapter (нужна первая реальная выгрузка от папы) → 2.6 e2e на 5 фирмах.

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence** (2.5.1 ✓ → 2.5.2 ✓ → 2.5.3 ✓ → 2.5.4 ✓ → 2.5.5 ✓ → 2.5.6 backend ✓ → 2.5.6.b frontend draft ✓ → 2.5.7 testcontainers ✓ → **2.5.8 ADR + DoD** ← здесь) → 2.3 SoliqExcelAdapter (после первой реальной выгрузки) → 2.6 E2E на 5 фирмах. VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
