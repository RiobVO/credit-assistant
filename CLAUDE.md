...
**Phase:** 2 — Data Adapters. Подфаза 2.5 (persistence) смержена в `main` через PR #1. **2.3 SoliqXltxAdapter в работе:** Day 1 (парсеры) ✓ запушен в `feat/soliq-xltx-vat-adapter`, не смержен. Day 2 + Day 3 — отдельные сессии.
**Last completed task:** 2.3 Day 1 — SoliqXltxAdapter parsers (commit `dab56d4`). Папа прислал 5 xltx-форм за свою фирму (декларация НДС март 2026 + ilova-реестр ЭСФ построчно с НДС + Form №2 + Form №1 + Расчёт прибыли). Реализованы парсеры VAT-декларации и ilova-реестра, плюс format_detector на все 5 типов и общий header_parser. 51 unit-тест на synthetic factory workbooks (без xltx-бинарников в git), real-data smoke на файлах папы: declared 62.8M ↔ ilova sum 63.5M, диф 1.19% (фирма здоровая, правило молчит). Ruff + mypy --strict + 334 теста зелёные.
**Next task:** **2.3 Day 2** — domain integration. Дизайн-решение: расширить `BorrowerSnapshot` для месячных VAT-периодов (новое поле или допущение в существующем VO), мапить выход парсеров в `SoliqChunk` (или новый chunk), update правила `VAT_ESF_MISMATCH` под месячный период. Затем e2e smoke: декларация + ilova + CSV папы → snapshot → правило. **2.3 Day 3** — API endpoint `POST /api/upload/soliq-xltx` (multipart) + UI Step 2 «Загрузить из Soliq» + ADR 0006 + финальный CLAUDE.md update. Phase 4 (Bank Mode + SSO) — отдельный roadmap.

**Phase 2 декомпозиция (статус):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → 2.5 persistence ✓ → main (2.5.1 → 2.5.2 → 2.5.3 → 2.5.4 → 2.5.5 → 2.5.6 → 2.5.6.b → 2.5.7 → 2.5.8) → **2.3 SoliqXltxAdapter в работе** (Day 1 parsers ✓ → Day 2 domain ⏸ → Day 3 API+UI+ADR ⏸) → 2.6 E2E на 5 фирмах ⏸ (pending: данные 5 фирм).
...
**Активная ветка:** `feat/soliq-xltx-vat-adapter` — пушнута в origin (`dab56d4`), tracked. Не смержена в main. Day 2 продолжит на той же ветке; PR на main после Day 3.

**Договорённости по Phase 2 (зафиксированы):**
- VAT хранится отдельно от ЭСФ — агрегат на `BorrowerSnapshot.esf_seller_vat_total` (ADR 0004). **2.3 Day 1 расширение:** реальный источник этого агрегата — ilova-приложение №4 к Расчёту НДС (детальный реестр ЭСФ построчно с НДС). Декларация даёт `vat_declared` (стр.0101 list02), ilova даёт `esf_seller_vat_total`. Числа сходятся между источниками до копейки.
