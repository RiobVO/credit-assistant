...
**Last completed task:** 2.5.1 Alembic + async engine — `sqlalchemy[asyncio]>=2.0.36`, `asyncpg>=0.30`, `alembic>=1.14`, `testcontainers[postgres]>=4.8` (dev). `Settings.database_url` + `draft_ttl_days=30`. `infrastructure/persistence/database.py` — `Base(DeclarativeBase)`, lazy `get_engine()` / `get_session_factory()`, FastAPI dep `get_session()`, `dispose_engine()`. `alembic.ini` (script_location → `src/infrastructure/persistence/migrations`, prepend_sys_path=src, ruff post-write hooks, sqlalchemy.url пуст — URL приходит из Settings). `env.py` — async через `async_engine_from_config` + `run_sync`, target_metadata=Base.metadata, WindowsSelectorEventLoopPolicy для Windows (asyncpg несовместим с ProactorEventLoop). Compose-postgres переведён на host port **5433** (локальный нативный Postgres на 5432 рвал соединения). Smoke: `alembic current` зелёный против 5433. ruff + mypy + 264 теста — зелёные.
**Next task:** 2.5.2 ORM models — `BorrowerORM`, `BorrowerSnapshotORM`, `DossierORM`, `DraftORM` в `infrastructure/persistence/models/`, маппинг 1:1 с domain через mappers. Подтянуть импорты в `env.py` для autogenerate.

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence** (2.5.1 ✓ → **2.5.2** ← здесь → 2.5.3 initial migration → 2.5.4 repos → 2.5.5 wire endpoint + CA-005 → 2.5.6 drafts → 2.5.7 testcontainers → 2.5.8 ADR + DoD) → 2.3 SoliqExcelAdapter (после первой реальной выгрузки) → 2.6 E2E на 5 фирмах. VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
