...
**Phase:** 2 — Data Adapters. Подфаза 2.5 (persistence) — закрыта; 2.3 + 2.6 в pending-external-data.
**Last completed task:** 2.5.8 — ADR 0005 (`docs/adr/0005-drafts-auth-by-uuid.md`) фиксирует модель auth черновиков на Phase 2: UUID4 как identifier, без owner-поля, TTL=30d, payload `dict[str, Any]` без strict-валидации (final-валидация на финальном POST). Угрозы и mitigation таблично; Phase 4 plan — добавить `owner_user_id` при появлении Bank SSO, держать UUID-only fallback для Accountant Mode без SSO. ADR содержит Definition of Done подфазы 2.5 (12 пунктов, все ✅) и open items (CA-003/004/006, scheduled purge job — не блокируют закрытие). Verify: только docs, runtime не затронут, 300 тестов прежние; ADR прочитан глазами на consistency с актуальным кодом. Коммит — следующим шагом после CLAUDE.md sync.
**Next task:** Phase 2 closure — два варианта на выбор пользователя: (а) PR на main «Phase 2.5 persistence complete», 2.3/2.6 остаются в backlog ветки `feat/...` после merge; (б) ветка остаётся открытой до прихода реальной выгрузки от папы, 2.3 SoliqExcelAdapter и 2.6 e2e на 5 фирмах добавляются в эту же ветку, потом один большой PR. Решение — за пользователем.

**Phase 2 декомпозиция (согласована):** 2.0 ✓ → 2.1 ✓ → 2.2 ✓ → 2.4.1 ✓ (API) → 2.4.2 ✓ (UI) → **2.5 persistence ✓** (2.5.1 → 2.5.2 → 2.5.3 → 2.5.4 → 2.5.5 → 2.5.6 backend → 2.5.6.b frontend draft → 2.5.7 testcontainers → 2.5.8 ADR + DoD) → 2.3 SoliqExcelAdapter (pending: первая реальная выгрузка от папы) → 2.6 E2E на 5 фирмах (pending: данные 5 фирм). VAT-адаптер для активации `VAT_ESF_MISMATCH` появится в 2.x после получения сводной справки НДС.
