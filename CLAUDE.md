...
- TODO[CA-041] (P2): wiring парсера FORM_1 в `parse_manual_input_files` use case + `applyToForm` в `parsed-files-dropzone.tsx`. Сейчас FORM_1 парсер реализован (CA-029a, `2bf833b`), но use case при FORM_1 файле эмитит только warning. После wiring `form1.assets_total` / `form1.liabilities_total` начнут попадать в source_trail → unlock COMPREHENSIVE readiness для пользователей с полным набором.
