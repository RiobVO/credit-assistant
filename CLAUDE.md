...
**T3.6 (On-prem tarball deploy) complete 2026-05-18** (commits `9a4cef1` + `eaecec5` + `5684f22` + `a3c2d1a` + `a3781c2`). 5 атомарных коммитов. **Deal-breaker** zero-internet install для узб mid-tier банков closed. ADR-0021. **T3.6.1** `web/Dockerfile` multi-stage (deps/builder/runner) + `next.config.ts` `output: "standalone"` — image ~200-250 MB, USER nextjs (UID 1001 non-root), NEXT_TELEMETRY_DISABLED. **T3.6.2** `deploy/install.sh` (preflight → `.env` validation → `docker load` → systemd install + enable → wait /health 90s). `deploy/.env.example` (все обязательные env с placeholder'ами). `deploy/Caddyfile.template` (reverse proxy + HSTS/X-Frame/Referrer-Policy baseline). `deploy/systemd/credit-assistant.service` + `credit-restore-drill.{service,timer}` (Sun 04:00 Asia/Tashkent weekly). `docker-compose.prod.yml` override (postgres/redis без host-portов, caddy 80/443). **T3.6.3** `scripts/build_release_tarball.sh` — bundles credit-assistant-vX.Y.Z.tar.gz: 5 docker save -gz images + source + scripts/ + config/ + deploy/ + compose + docs/ + SHA256. ~600 MB - 1.5 GB. **T3.6.4** ADR-0021 (alternatives rejected: registry-pull / Ansible / K8s / apt-repo). `deploy/README.md` "0 → running" walkthrough <30 мин: preconditions / extract + secrets / install.sh / verify / upgrade / troubleshooting / uninstall soft+hard.

**T3.3 (Prometheus /metrics + Grafana dashboard) complete 2026-05-18** (commits `daea4f4` + `9135ccb` + `5da48ac` + this). 4 атомарных коммита. ADR-0023. **Tier 3 fully closed (6/6).** **T3.3.1** Backend `prometheus-fastapi-instrumentator>=7`. Auto HTTP метрики: `http_requests_total` / `http_request_duration_seconds` (histogram) / `http_inprogress_requests` (gauge). 3 custom: `pdf_render_duration_seconds` (histogram buckets 0.5-60s), `parser_warnings_total{format}` (vat_declaration/vat_registry_ilova/form_1/form_2/profit_tax), `red_flags_fired_total{severity}` (critical/high/medium/low). `Settings.metrics_enabled: bool = False` default off (zero overhead в dev). `setup_prometheus(app)` conditional в `create_app`. excluded_handlers `["/metrics", "/health"]` — не считаем probe traffic. 5 unit tests. **T3.3.2** `docker-compose.metrics.yml` opt-in override (Prometheus 9090 + Grafana 3001, 30d retention). `deploy/metrics/`: prometheus scrape config + Grafana auto-provisioning (datasources + dashboards) + bundled dashboard JSON «Credit Assistant — Operations» с 7 panels (request rate / error rate / latency p50/p95/p99 / PDF render p50/p95 / parser warnings rate by format / red flags rate by severity / inflight). ADR-0023 (alternatives rejected: plain logs+jq / InfluxDB / Datadog SaaS / full OTel). Playbook `docs/operations/metrics.md` (9 sections: dev setup, dashboard tour, prod deploy options, alert rules pack, cross-reference event↔log↔metric через request_id, troubleshooting, operational checklist). **T3.3.3** custom counters wired в реальные call-sites: `WeasyPrintPdfRenderer.render` оборачивает write_pdf в `pdf_render_duration.time()`; `shared/dossier.py` create_dossier инкрементирует `red_flags_fired_total{severity}` per срабатывание; `parse_manual_input_files.execute` инкрементирует `parser_warnings_total{format}` на длину warnings. Lazy imports — application/interfaces избегают hard-coupling с observability infra. **T3.3.4** docs sync. **Tier 3 complete — переход к T4 compliance pack либо real-bank pilot.**

**T3.1 (Observability via GlitchTip on-prem) complete 2026-05-18** (commits `07864df` + `8973dbd` + `65009c9` + `a9ffc9b` + this). 4 атомарных коммита + lock-fix. ADR-0022. Sentry SaaS forbidden (PROJECT_BRIEF Sec 8 — data leaves периметр) → **GlitchTip on-prem** (OSS Sentry-protocol-compat). **T3.1.1** Backend `sentry-sdk[fastapi]>=2.20`. `init_sentry(settings)` в `create_app` ДО `configure_logging`. `send_default_pii=False`, `traces_sample_rate=0` (errors only, экономия storage), `before_send` PII scrubber (drops `request.data`/`cookies`, scrubs keys matching `password|token|secret|api[_-]?key|authorization|cookie|mfa|backup_codes|pii_enc|jwt`, маскирует emails через `email_mask` в breadcrumbs/extra/tags/headers). Integrations: FastApi + SqlAlchemy + Asyncio + Logging (event_level/level=None — не дублирует наш `setLogRecordFactory` из T3.2). Global tags: `brand_id`, `app_mode`, `app_env`. `RequestIDMiddleware` дополнительно вызывает `sentry_sdk.get_current_scope().set_tag("request_id", rid)` → cross-ref event ↔ structured log. 8 unit tests passed. **T3.1.2** Frontend `@sentry/nextjs ^10.53.1`. `web/instrumentation.ts` — server runtime register hook (`NEXT_RUNTIME=nodejs` → init если `SENTRY_DSN`). `web/instrumentation-client.ts` — browser init из `NEXT_PUBLIC_SENTRY_DSN`, `replaysSessionSampleRate=0` + `replaysOnErrorSampleRate=0` (banking UI privacy). `web/src/app/error.tsx` — `console.error` + `TODO[CA-064]` заменены на `Sentry.captureException(error, { tags: { digest: error.digest } })`. `withSentryConfig` НЕ wrap'нут — sourcemaps upload requires CI release pipeline (post-demo). Build/vitest 216 passed. **T3.1.3** `docker-compose.observability.yml` opt-in override — 4 контейнера (`glitchtip-postgres` + `glitchtip-redis` + `glitchtip-web` `:8001` + `glitchtip-worker` celery). ADR-0022 (alternatives rejected: SaaS Sentry / plain logs+SIEM / Highlight / Errbit). `docs/operations/observability.md` playbook: dev setup, PII review, prod deploy (bundled Caddy proxy vs отдельный host), alert rules, cross-reference event ↔ log через request_id, troubleshooting, operational checklist. `deploy/.env.example` дополнен `SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN` placeholders. **T3.1.4** docs sync. **Tier 3 — 5 из 6 closed. Остался T3.3 Prometheus.**

**T3.5 (Audit-log CSV export) complete 2026-05-18** (commit `9c02171`). 1 atomic commit. `GET /api/bank/admin/audit-log/export?from=&to=&event=` → StreamingResponse CSV. Schema `AuditExportQuery` (ISO8601 UTC `[from, to)` semi-open, optional event filter, 422 на инверсию дат). Repository `export_range(from_, to, event)` — async list (audit ≤100K rows не требует DB-level streaming; endpoint stream'ит CSV-bytes). brand_id-инвариант через repo constructor. CSV columns: id / created_at / brand_id / request_id (T3.2) / event / analyst_id / target_type / target_id / payload JSON. Сам факт экспорта пишется в audit_log как `audit_log_export` с payload {from, to, event_filter, rows_count} **до** stream'а (запись попадает в БД даже если клиент оборвёт connection). Senior-analyst guard через `SeniorAnalystDep`. 5 integration tests passed (happy / event filter / forbidden / invalid range 422 / self-logs).

**Tier 3 progress: 4 из 6 closed** (T3.2 / T3.4 / T3.5 / T3.6). Остались **T3.1 observability** (GlitchTip on-prem) + **T3.3 Prometheus**.

**T3.4 (Postgres backup + restore drill) complete 2026-05-18** (commits `4f333c5` + `7ccdbba` + `e5fdd98` + `9022451` + `ffc3b42`). **Deal-breaker** для bank-IT onboarding closed. `scripts/backup_postgres.sh` — bash, `pg_dump --format=custom --compress=9 --no-owner --no-acl` + retention `find -mtime +N` (default 7d dev / 30d prod через env). Encryption opt-in через `BACKUP_AGE_RECIPIENT` (age — modern asymm, single binary, no keyring). Exit-codes контракт 10/11/12. `scripts/restore_drill.sh` — pg_restore latest в temp DB → COUNT(*) per table source vs restored → PASS если max diff < `DRILL_ROW_DIFF_PCT` (default 5%). Cleanup temp DB через `trap EXIT`. `docker-compose.backup.yml` sidecar — Windows-Docker compat fix (`ffc3b42`): bash sleep-loop вместо dcron (setpgid: Operation not permitted в WSL2), busybox `find` → `ls -t` fallback, `git update-index --chmod=+x` на скрипты для host-bind. `docs/operations/db-backup.md` — playbook: 3 prod destinations (A local / B NFS / C S3-compat MinIO), encryption setup, disaster scenarios, operational checklist, systemd timer для weekly drill. Tests: 3 integration backup + 3 restore drill (testcontainers, **passed в CI на ubuntu-latest** — Docker + pg_dump preinstalled). Local Windows smoke: backup создал 2 dump'а, restore_drill PASS exit=0 («max row diff 0%»).
...
### Tier 3 ✅ closed 2026-05-18 — Operational readiness complete

Все 6 items закрыты в одной сессии (14+ atomic commits). См. Current Status
выше для деталей каждого. Pre-demo MVP ready: deal-breakers (case_id /
PII / multi-tenant / LDAP / data quality / backup / on-prem deploy /
observability stack) и nice-to-have (audit-export / metrics) сняты.

**Next:** T4 compliance pack (параллельно), real-bank pilot trip, либо
T3 hardening backlog (CI Docker job optional, release pipeline для
Sentry sourcemaps upload, alert-rules как код через AlertManager).
...
1. ~~**T3.2** Structured logging~~ ✅ **DONE 2026-05-18** — `correlation_id` через `setLogRecordFactory` + `RequestIDMiddleware` + `audit_log.request_id`.
2. ~~**T3.4** Postgres backup~~ ✅ **DONE 2026-05-18** — `backup_postgres.sh` + `restore_drill.sh` + sidecar + playbook (3 prod destinations).
3. ~~**T3.6** On-prem deploy~~ ✅ **DONE 2026-05-18** — bundled tarball builder + install.sh + Caddy + systemd + ADR-0021 + deploy/README.md "0 → running".
4. ~~**T3.5** Audit log export~~ ✅ **DONE 2026-05-18** — `GET /api/bank/admin/audit-log/export?from=&to=&event=` → CSV StreamingResponse + self-audit log.
5. ~~**T3.1** Observability~~ ✅ **DONE 2026-05-18** — GlitchTip on-prem (ADR-0022), sentry-sdk backend + @sentry/nextjs frontend, PII scrubber, request_id tag.
6. ~~**T3.3** Prometheus + Grafana~~ ✅ **DONE 2026-05-18** — ADR-0023, auto HTTP + 3 custom counters wired в call-sites, bundled Grafana dashboard.
